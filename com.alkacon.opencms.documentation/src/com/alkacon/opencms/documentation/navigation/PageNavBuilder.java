 ...
 */
/**
 * Builder for a page-navigation in the documentation. Builds up a list of
 * navigation elements similar to the OpenCms navigation mechanism.
 */
public class PageNavBuilder {
	...
	/**
	 * CmsObject for the request context in which the page for which the
	 * navigation is build is accessed.
	 */
	private CmsObject m_cmsObject;

	/**
	 * Type-id of the sections that should appear in the navigation. Initialized
	 * lazily.
	 */
	...
	/**
	 * @param cmsBean
	 *            the current CmsJspStandardContextBean
	 * @param containerName
	 *            name of the container where the content that should be
	 *            navigated is in.
	 */
	public PageNavBuilder(final CmsJspStandardContextBean cmsBean,
	...
	private void collectNavElementsInternal() throws CmsException {
		...
		for (CmsContainerElementBean elem : m_container.getElements()) {
			...
			if (isSection(elemResource)) {
				...
				// the following works because the element setting is present
				// only if explicitly set
				// default values are not present, thus take the default value
				// if the setting is not present.
				int level = levelString != null ? Integer.valueOf(levelString)
						.intValue() : 1;
	...
	/**
	 * @param elem
	 *            container element where the resource should be returned from
	 * @return resource of the container element
	 * @throws CmsException
	 */
	private CmsResource getElemResource(final CmsContainerElementBean elem)
			throws CmsException {
	 ...
	 */
	private boolean isSection(final CmsResource resource)
			throws CmsLoaderException {
	...
	private void addSectionToPageNav(final CmsResource section, final int level) {
		...
		CmsJspContentAccessBean sectionContent = new CmsJspContentAccessBean(
				m_cmsObject, section);
		String index = m_indexer.getSectionIndex(level);
		String title = sectionContent.getValue().get("Headline").toString();
