        ...
        <c:otherwise>
            <div class="row">
                <aside class="col-lg-3 hidden-md hidden-sm hidden-xs">
                    ...
                    <c:set var="folders" value="${fn:split(fn:substringAfter(cms.requestContext.uri,'/'),'/')}" />
                    <c:choose>
                    <c:when test="${folders[0] eq 'documentation' and fn:length(folders) > 2}"> <%--OCEE --%>
                        <c:set var="siteNavBasePath">/${folders[0]}/${folders[1]}</c:set>
                        <c:set var="navStartLevel">2</c:set>
                    </c:when>
                    <c:otherwise>
                        <c:set var="siteNavBasePath">/${folders[0]}</c:set>
                    ...
                    <cms:include file="%(link.strong:/system/modules/com.alkacon.opencms.documentation/elements/sitenavigation.jsp:8d667ace-2930-11e4-b03d-d144f6bb3566)">
                        <cms:param name="resource">${siteNavBasePath}</cms:param>
                        <cms:param name="navStartLevel">${navStartLevel}</cms:param>
                    </cms:include>
                    <!-- END: Site Navigation -->
